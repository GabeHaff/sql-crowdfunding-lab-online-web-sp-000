...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  "SELECT projects.title, (SUM(pledges.amount)-projects.funding_goal) AS total_funds FROM projects
  JOIN pledges ON pledges.project_id = projects.id  
  GROUP BY projects.title HAVING total_funds >=0 
...
def selects_the_category_names_and_pledge_amounts_of_all_pledges_in_the_music_category
  ...
  JOIN projects.category ON pledges.project_id = project.id 
  WHERE category = 'music' GROUP BY projects.category 
